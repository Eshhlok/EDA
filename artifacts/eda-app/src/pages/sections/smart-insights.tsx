...
const isDateLikeColumn = (col: string) => {
  ...
  return (
    lower.includes("_month") || lower.includes("_year") || lower.includes("_quarter") ||
    lower.includes("month") || lower.includes("year") || lower.includes("quarter") ||
    lower.includes("date") || /\d{4}/.test(col)
  );
};

export default function SmartInsights({ datasetId }: Props) {
  ...
  const timeColumns = categoricalColumns.filter((col) => isDateLikeColumn(col));

  const selectedMetric =
    numericColumns.find((col) =>
      col.toLowerCase().includes("cost") || col.toLowerCase().includes("sales") ||
      col.toLowerCase().includes("revenue") || col.toLowerCase().includes("amount")
    ) || numericColumns[0];

  const categoryPriority = [
    "department", "cost center", "division", "plant", "unit",
    "team", "region", "product", "category", "segment",
  ];

  const selectedCategory =
    categoricalColumns.find((col) =>
      categoryPriority.some((keyword) => col.toLowerCase().includes(keyword))
    ) ||
  ...
  const isFinancial = ["cost", "expense", "revenue", "sales", "amount", "spend", "price", "profit", "income"]
    .some((word) => metricLower.includes(word));
  const isOperational = ["department", "cost center", "division", "plant", "unit", "team", "facility"]
    .some((word) => categoryLower.includes(word));
  const isGeographical = ["region", "city", "country", "state", "zone", "territory"]
    .some((word) => categoryLower.includes(word));
  const isProduct = ["product", "item", "sku", "inventory", "material", "asset"]
    .some((word) => categoryLower.includes(word));

  const metricLabel = isFinancial ? "expenditure" : "metric value";
  const categoryLabel = isOperational ? "department" : isGeographical ? "region" : isProduct ? "product category" : "category";
  const concentrationLabel = isOperational ? "Operational Concentration" : isGeographical ? "Regional Concentration" : isProduct ? "Product Concentration" : "Distribution Concentration";
  ...
  useEffect(() => {
    async function fetchInsights() {
      try {
        if (!selectedMetric || !selectedCategory || !selectedTime) return;
        setLoading(true);
        ...
        setMonthlyData(await monthRes.json());
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const insights = useMemo(() => {
    if (!departmentData?.data || !monthlyData?.data) return [];

    const results: { title: string; description: string; severity: "info" | "warning" | "success" }[] = [];

    const totalSpend = departmentData.data.reduce((sum: number, row: any) => sum + row.value, 0);
    results.push({
    ...
    const topDept = [...departmentData.data].sort((a, b) => b.value - a.value)[0];
    const topPct = ((topDept.value / totalSpend) * 100).toFixed(1);
    const capitalizedCategoryLabel = categoryLabel.charAt(0).toUpperCase() + categoryLabel.slice(1);
    results.push({
    ...
    const top3 = [...departmentData.data].sort((a, b) => b.value - a.value).slice(0, 3);
    const top3Total = top3.reduce((sum: number, row: any) => sum + row.value, 0);
    ...
    const highestMonth = [...sortedMonths].sort((a, b) => b.value - a.value)[0];
    results.push({
    ...
    const mean = sortedMonths.reduce((s: number, r: any) => s + r.value, 0) / sortedMonths.length;
    const stdDev = Math.sqrt(
      sortedMonths.reduce((s: number, r: any) => s + Math.pow(r.value - mean, 2), 0) / sortedMonths.length
    );
    ...
    const topAnomaly = scoredMonths[0];
    ...
    results.push({
      title: isOperational ? "Operational Recommendation" : isGeographical ? "Regional Recommendation" : isProduct ? "Product Recommendation" : "Optimization Recommendation",
