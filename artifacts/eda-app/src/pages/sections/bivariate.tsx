...
export default function Bivariate({ datasetId }: { datasetId: string }) {
  ...
  useEffect(() => {
    if (bivariate) {
      if (!colX && bivariate.numeric_columns.length > 0) {
        setColX(bivariate.numeric_columns[0]);
      }
      if (!colY && bivariate.numeric_columns.length > 1) {
        setColY(bivariate.numeric_columns[1]);
      } else if (!colY && bivariate.numeric_columns.length > 0) {
        setColY(bivariate.numeric_columns[0]);
      }
    }
  }, [bivariate, colX, colY]);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Label htmlFor="spearman" className="text-sm text-muted-foreground">
              Pearson
            </Label>
            <Switch
              id="spearman"
              checked={useSpearman}
              onCheckedChange={setUseSpearman}
            />
            <Label htmlFor="spearman" className="text-sm text-muted-foreground">
              Spearman
            </Label>
...
function ScatterPlotData({
  ...
  const { data, isLoading } = useGetScatterData(
    datasetId,
    { col_x: colX, col_y: colY },
    { query: { enabled: !!colX && !!colY } }
  );
  ...
  const chartData = data.x
    .map((xVal, i) => ({ x: xVal, y: data.y[i], color: data.color?.[i] }))
  ...
  return (
    <div className="space-y-4">
      <div className="h-[400px] border border-border/50 rounded-2xl bg-card/40 backdrop-blur-sm p-2 shadow-lg">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <Scatter data={chartData} fill="hsl(var(--primary))" opacity={0.75} stroke="none" />
          </ScatterChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex gap-4 text-sm bg-muted/50 p-3 rounded-xl border border-border/40">
        {data.pearson_r !== null && (
          <div>
            <span className="font-semibold">Pearson r:</span>{" "}
            <span className="font-mono">{data.pearson_r?.toFixed(3)}</span>
          </div>
        )}
        {data.spearman_r !== null && (
          <div>
            <span className="font-semibold">Spearman r:</span>{" "}
            <span className="font-mono">{data.spearman_r?.toFixed(3)}</span>
          </div>
        )}
      </div>
    </div>
